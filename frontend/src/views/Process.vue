            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After the documents are uploaded, the LLM analyzes the content and generates the ontology structure for the simulation automatically (entity types + relation types).
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">Endpoint Overview</div>
                <div class="detail-content">
                  Based on the generated ontology, the documents are chunked and sent to the Zep API to build the knowledge graph and extract entities and relations.
                </div>
              </div>
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'No files are queued for upload. Return to the homepage and try again.'
    loading.value = false
    return
  }
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      buildProgress.value.message = 'Knowledge graph build task started...'
      ...
      error.value = response.error || 'Failed to start knowledge graph build'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'Failed to start knowledge graph build: ' + (err.message || 'Unknown error')
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'Knowledge graph build failed: ' + (task.error || 'Unknown error')
        buildProgress.value = null
      }
    }
  } catch (err) {
